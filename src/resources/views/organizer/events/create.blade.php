<x-organizer-layout>
    <x-slot name="header">Buat Event Baru</x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="px-6 py-4 border-b border-gray-100">
                <h1 class="text-base font-semibold text-gray-900">Buat Event Baru</h1>
                <p class="text-sm text-gray-500 mt-0.5">Isi detail event yang akan kamu buat.</p>
            </div>
            ...
            <form method="POST" action="{{ route('organizer.events.store') }}"
                ...
                <div>
                    ...
                    {{-- Preview --}}
                    <div id="image-preview" class="hidden mb-3 relative group cursor-pointer" onclick="document.getElementById('banner_image').click()">
                    ...
                    <label id="upload-zone" for="banner_image"
                           class="flex flex-col items-center justify-center w-full h-44 border-2 border-dashed rounded-xl cursor-pointer transition-colors
                                  {{ $errors->has('banner_image') ? 'border-red-400 bg-red-50' : 'border-gray-300 hover:border-indigo-400 hover:bg-indigo-50' }}">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              placeholder="Ceritakan tentang event ini — siapa yang tampil, apa yang bisa dinikmati, kenapa harus hadir..."
                              class="w-full px-3 py-2.5 text-sm border rounded-xl transition-colors
                                     {{ $errors->has('description') ? 'border-red-400 bg-red-50' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500' }}
                                     focus:outline-none focus:ring-1">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location" value="{{ old('location') }}"
                           placeholder="Contoh: Gelora Bung Karno, Jakarta Pusat"
                           class="w-full px-3 py-2.5 text-sm border rounded-xl transition-colors
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="ticket_price" name="ticket_price"
                                   value="{{ old('ticket_price', 0) }}" min="0" step="1000"
                        ...
                        <p class="mt-1 text-xs text-gray-400">Isi 0 jika event gratis</p>
                        @error('ticket_price')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <label class="flex-1 flex items-start gap-3 p-3 bg-white border-2 rounded-xl cursor-pointer transition-colors
                                      {{ old('status', 'draft') === 'draft' ? 'border-indigo-500' : 'border-gray-200 hover:border-gray-300' }}">
                            <input type="radio" name="status" value="draft"
                                   {{ old('status', 'draft') === 'draft' ? 'checked' : '' }}
                                   class="mt-0.5 text-indigo-600 focus:ring-indigo-500">
                            <div>
                                <p class="text-sm font-semibold text-gray-900">Draft</p>
                                <p class="text-xs text-gray-500 mt-0.5">Simpan dulu, belum publik</p>
                            </div>
                        </label>
                        <label class="flex-1 flex items-start gap-3 p-3 bg-white border-2 rounded-xl cursor-pointer transition-colors
                                      {{ old('status') === 'published' ? 'border-green-500' : 'border-gray-200 hover:border-gray-300' }}">
                            <input type="radio" name="status" value="published"
                                   {{ old('status') === 'published' ? 'checked' : '' }}
                                   class="mt-0.5 text-green-600 focus:ring-green-500">
                            <div>
                                <p class="text-sm font-semibold text-gray-900">Publish</p>
                                <p class="text-xs text-gray-500 mt-0.5">Langsung tampil ke publik</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <a href="{{ route('organizer.events.index') }}"
                       class="px-4 py-2.5 text-sm font-medium text-gray-600 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Batal
                    </a>
                    <button type="submit"
